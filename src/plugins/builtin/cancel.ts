...
/**
 * `/cancel` — stops any active ralph-loop by clearing the promise on TeamLead.
 * Also useful as a general "stop current operation" command.
 */
export const CANCEL_PLUGIN: Plugin = {
  name: 'cancel',
  command: '/cancel',
  description: 'Cancel the active ralph-loop or pending operation',
  handler: 'builtin',
  builtinKey: 'cancel',
  body: '',
  source: 'builtin',
...
export const cancelBuiltin: BuiltinHandler = async (ctx) => {
  ...
  ctx.emit('plugin_output', {
    stream: 'stdout',
    text: '[/cancel] cleared active ralph-loop (if any).',
  });
};
